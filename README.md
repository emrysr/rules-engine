...
| Config | Shape | Does |
| --- | --- | --- |
| **Data sources** | `{ key, url }` | `key` is the namespace rules target, and the response field the array is auto-extracted from. Each source is a fieldset in the Data Sources panel — rename, move, add or delete it, set its URL, fetch it on its own and preview what came back. |
