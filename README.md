...
| **Result** | `result?: "<pipeline name>"` | The one answer, shown in the Results panel: the named pipeline's result, or the last pipeline's. A pipeline can be switched off (`off: true`, the checkbox by its name) to test the result without it; the first is always on. Each pipeline folds away by clicking its title bar. |
